...
namespace DispatchSystem.Server
{
    public partial class DispatchSystem
    {
        ...
        public static void SetName(string handle, string first, string last)
        {
            ...
            Officer o = GetOfficer(handle);
            if (o != null && o.Status == OfficerStatus.OnDuty) // checking if the civilian has an officer
            {
                SendMessage(p, "DispatchSystem", new[] { 0, 0, 0 }, "You cannot be an onduty officer and a civilian at the same time!");
        ...
        public static void SetVehicle(string handle, string plate)
        {
            ...
            if (GetCivilianVehByPlate(plate) != null && GetPlayerByLic(GetCivilianVeh(handle).License) != p)
            {
        ...
        public static void TicketCivilian(string invokerHandle, string first, string last, string reason, float amount)
        {
            ...
            if (civ != null)
            {
                ...
                Player p = GetPlayerByLic(Civs[index].License); // finding the player that the civ owns
